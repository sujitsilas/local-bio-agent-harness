...
# ── timepoint setup + compartment restriction ──────────────────────────────────
# Find the unique timepoints in the dataset
ALL_TIMEPOINTS = ["D7", "D10", "D14", "D19"]
print(f'{len(ALL_TIMEPOINTS)} timepoints: {ALL_TIMEPOINTS}')

# ── 1. DE per timepoint (contamination-cleaned; X already log-normalized) ───────
de_mac_timepoint = {}
for tp in ALL_TIMEPOINTS:
    # Filter for the current timepoint AND cells within the macrophage compartment
    sel = (adata_full.obs['Timepoint'] == tp).values & in_compartment
    adata_tp = adata_full[sel].copy()
    
    n_b = (adata_tp.obs['Type'] == 'Burn').sum()
    n_s = (adata_tp.obs['Type'] == 'Sham').sum()
    print(f'\nTimepoint {tp} — {adata_tp.n_obs} cells  Burn={n_b}  Sham={n_s}')
    
    if n_b < 3 or n_s < 3:
        print('  Skipping — too few cells.'); de_mac_timepoint[tp] = None; continue

    # Keep "Macrophages" as the target celltype to use the same contamination filter
    adata_tp = prepare_celltype_for_DE(adata_tp, "Macrophages", normalize=False)
    
    sc.tl.rank_genes_groups(adata_tp, groupby='Type', groups=['Burn'], reference='Sham',
                            method='wilcoxon', use_raw=False, pts=True, key_added='rgg')
    
    df = sc.get.rank_genes_groups_df(adata_tp, group='Burn', key='rgg')
    ...
    de_mac_timepoint[tp] = df
    
    nb = ((df.padj < FDR_THRESH) & (df.lfc > LFC_THRESH)).sum()
    ns = ((df.padj < FDR_THRESH) & (df.lfc < -LFC_THRESH)).sum()
    print(f'  Burn↑: {nb}   Sham↑: {ns}')
    
    df.to_csv(FIGDIR_MAC / f'de_timepoint_{_slug(str(tp))}_burn_vs_sham.csv', index=False)


# ── 2. Volcano grid ───────────────────────────────────────────────────────────
MUST_LABEL = ["Arg1", "Nos2", "Il1b", "Il1", "Il33"]

# Switch to using the new timepoint dictionary
valid_tps = [tp for tp in ALL_TIMEPOINTS if de_mac_timepoint.get(tp) is not None]
ncols = min(4, len(valid_tps)); nrows = int(np.ceil(len(valid_tps) / ncols))
fig, axes = plt.subplots(nrows, ncols, figsize=(7 * ncols, 7 * nrows))
axes_flat = np.atleast_1d(axes).flatten()
N_LABEL_DRAW = min(N_LABEL, 12)

for ax_idx, tp in enumerate(valid_tps):
    ax = axes_flat[ax_idx]; df = de_mac_timepoint[tp]
    ...
    count_burn = ax.text(0.03, 0.86, f'Burn↑ {nb_}', transform=ax.transAxes,
                         fontsize=15, va='top', ha='left', color=BURN_COL, fontweight='bold')
    count_sham = ax.text(0.03, 0.81, f'Sham↑ {ns_}', transform=ax.transAxes,
    ...
    ax.set_title(f'Timepoint: {tp}', fontsize=30, fontweight='bold'); ax.grid(False)

for ax in axes_flat[len(valid_tps):]:
    ax.set_visible(False)
fig.tight_layout(); fig.canvas.draw()
fig.savefig(FIGDIR_MAC / 'volcano_timepoint_burn_vs_sham.pdf', dpi=300, bbox_inches='tight')
fig.savefig(FIGDIR_MAC / 'volcano_timepoint_burn_vs_sham.png', dpi=300, bbox_inches='tight')
plt.show(); plt.close(fig)
print('Saved: volcano_timepoint_burn_vs_sham.{pdf,png}')


# ── 3. GO enrichment tile — one per timepoint ──────────────────────────────────
for tp in valid_tps:
    df = de_mac_timepoint[tp]
    ...
    for direction, glist in gene_sets.items():
        print(f'  Timepoint {tp} | {direction}: {len(glist)} genes')
        if len(glist) < 5:
            continue
        try:
    ...
    make_enrichment_tile(pd.DataFrame(rows),
                         output_name=FIGDIR_MAC / f'enrichment_tile_timepoint_{_slug(str(tp))}.pdf',
                         title=f'GO Biological Process — Timepoint: {tp}\nBurn vs Sham',
                         tile_size=0.7)
