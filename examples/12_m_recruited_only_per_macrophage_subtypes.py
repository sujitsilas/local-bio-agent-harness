...
# ══════════════════════════════════════════════════════════════════════════════
RECRUITED_IDENT = "MΦ-Resident/Repair"
...
print(f"MΦ-Recruited subset: {adata_recr.n_obs} cells")
...
for sub in SUBTYPES:
    ...
    for i, tp in enumerate(valid):
        draw_volcano(axes_flat[i], de_recr_sub[sub][tp], f"MΦ-Resident/Repair· {sub}\n{tp}")
    ...
    print(f"Saved volcano: MΦ-Resident/Repair · {sub}")
...
for sub in SUBTYPES:
    for tp in ALL_TIMEPOINTS:
        ...
        make_enrichment_tile(pd.DataFrame(rows),
                             ...
                             title=f"MΦ-Resident/Repair · {sub} — {tp}\nBurn vs Sham GO BP", tile_size=0.7)
