...
# ══════════════════════════════════════════════════════════════════════════════
# Consecutive-timepoint DE (LATER vs EARLIER), per identity × condition:
#   D14 vs D10, D19 vs D14  for Inflammatory Monocytes & MΦ-Recruited,
...
PAIRS  = [("D7", "D10"), ("D14", "D10"), ("D19", "D14")]      # (later, earlier)
...
IDENTS, CONDS = [x for x in [INF, RECR, RESI] if x], ["Sham", "Burn"]

def _run_tp(ident, cond, later, earlier, min_pct=0.20):
    sel = ((_idv == ident).values &
           (adata_mac.obs["Type"].astype(str) == cond).values &
           (adata_mac.obs["Timepoint"].astype(str).isin([later, earlier])).values)
    ...
    df = sc.get.rank_genes_groups_df(a, group=later, key="rgg").dropna(subset=["logfoldchanges", "pvals_adj"])

    # keep genes detected in ≥ min_pct of cells in the LATER or EARLIER population
    ...
    if len(pct) >= 2:
        ...
        print(f"  detection filter (≥{int(min_pct*100)}% in either group): {n_before} → {len(df)} genes")
